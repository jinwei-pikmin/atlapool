...
- `description_append`（`Value`，可為 string 或 ADF content node）：append-only；工具內部完成 GET → merge → PUT。
...
3. 在 `content` 陣列尾端加入新的 ADF content node：
   - 若 `description_append` 是 string，包成 `{"type":"paragraph","content":[{"type":"text","text":"..."}]}`。
   - 若 `description_append` 是 object，必須有 `type` 欄位，直接 append。
...
- `description_append` 的 ADF node 格式尚未與 Mario agent 的 prompt 慣例對齊；目前採用「string → paragraph」的保守預設，未來可接受完整 ADF object。
